...
interface Row {
  ...
  teacher_id: string | null; plan_started_at: string | null; plan_expires_at: string | null; is_active: boolean;
  profile: { full_name: string; email: string; phone: string | null; document: string | null; rg: string | null; birth_date: string | null; address: string | null; whatsapp: string | null } | null;
  plan: { id: string; name: string; description: string | null; price: number | null; duration_days: number; presential_per_week: number } | null;
}
...
function AlunosAdmin() {
  ...
  const [contract, setContract] = useState<{ template: string; aluno: ContractStudent; plano: ContractPlan; datas: ContractDates; name: string } | null>(null);
  ...
  const openContrato = async (r: Row) => {
    const { data: settings } = await supabase.from("app_settings").select("contract_template").eq("id", true).maybeSingle();
    const template = (settings?.contract_template as string | null) ?? "";
    if (!template.trim()) { toast.error("Configure o modelo de contrato em Contrato antes."); return; }
    setContract({
      template,
      ...
      plano: r.plan
        ? { name: r.plan.name, description: r.plan.description, price: r.plan.price, duration_days: r.plan.duration_days, presential_per_week: r.plan.presential_per_week }
        : {},
  ...
  const load = async () => {
    const { data, error } = await supabase.from("students")
      .select("id,user_id,total_points,plan_id,teacher_id,plan_started_at,plan_expires_at,is_active,plan:plans(id,name,description,price,duration_days,presential_per_week)")
      .order("created_at", { ascending: false });
    if (error) { toast.error(error.message); return; }
    const userIds = (data ?? []).map((s) => s.user_id);
    const { data: profs } = userIds.length
      ? await supabase.from("profiles").select("id,full_name,email,phone,document,rg,birth_date,address,whatsapp").in("id", userIds)
      : { data: [] as { id: string; full_name: string; email: string; phone: string | null; document: string | null; rg: string | null; birth_date: string | null; address: string | null; whatsapp: string | null }[] };
  ...
  const createStudent = async (e: FormEvent) => {
    ...
    setSaving(false);
    if (!res.ok) { toast.error(data.error ?? "Falha ao cadastrar"); return; }
  ...
  return (
    <div className="space-y-6">
      <div className="flex flex-wrap items-start justify-between gap-3">
        <div><h1 className="text-3xl font-bold">Alunos</h1><p className="text-muted-foreground">Cadastre manualmente ou gerencie planos, contratos e status.</p></div>
        <Button onClick={() => setCreating(true)} className="bg-gradient-primary text-primary-foreground"><Plus size={16} className="mr-1" /> Cadastrar Aluno</Button>
      </div>
              ...
              <tr key={r.id} className="border-t border-border">
                ...
                <td className="p-3 text-right">
                  <div className="flex justify-end gap-1">
                    <Button size="icon" variant="ghost" onClick={() => openContrato(r)} title="Ver contrato"><ScrollText size={14} /></Button>
        ...
        <DialogContent className="max-h-[90vh] overflow-y-auto sm:max-w-2xl">
          ...
          <form onSubmit={createStudent} className="space-y-3">
            <div className="grid grid-cols-2 gap-3">
              <div className="col-span-2"><Label>Nome completo *</Label><Input name="full_name" required /></div>
              <div><Label>Email *</Label><Input name="email" type="email" required /></div>
              <div><Label>Senha inicial *</Label><Input name="password" type="text" required minLength={6} placeholder="mín. 6 caracteres" /></div>
              <div><Label>Telefone/WhatsApp</Label><Input name="phone" /></div>
              <div><Label>Data de nascimento</Label><Input name="birth_date" type="date" /></div>
              <div><Label>CPF</Label><Input name="document" /></div>
              <div><Label>RG</Label><Input name="rg" /></div>
              <div className="col-span-2"><Label>Endereço</Label><Input name="address" /></div>
              <div>
                <Label>Plano</Label>
                <Select name="plan_id">
            ...
            <DialogFooter>
              ...
              <Button type="submit" disabled={saving} className="bg-gradient-primary text-primary-foreground">{saving ? "Cadastrando..." : "Cadastrar"}</Button>
            </DialogFooter>
          </form>
        </DialogContent>
      </Dialog>
      ...
      <Dialog open={!!contract} onOpenChange={(o) => !o && setContract(null)}>
        <DialogContent className="max-h-[95vh] overflow-hidden sm:max-w-4xl">
          <DialogHeader><DialogTitle>Contrato — {contract?.name}</DialogTitle></DialogHeader>
          {contract && (
            <ContractView template={contract.template} aluno={contract.aluno} plano={contract.plano} datas={contract.datas} />
          )}
        </DialogContent>
      </Dialog>
    </div>
