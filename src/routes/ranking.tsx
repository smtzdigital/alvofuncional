...
export const Route = createFileRoute("/ranking")({
  head: () => ({ meta: [{ title: "Ranking público — ForgeFit" }] }),
  component: PublicRanking,
});

interface RankRow { student_id: string; full_name: string; total_points: number; plan_name: string | null; rank: number; }

function PublicRanking() {
  const [rows, setRows] = useState<RankRow[]>([]);
  useEffect(() => { supabase.rpc("get_ranking", { _limit: 50 }).then(({ data }) => setRows((data ?? []) as RankRow[])); }, []);

  return (
    <div className="min-h-screen bg-gradient-hero">
      <header className="container mx-auto flex items-center justify-between px-6 py-6">
        <Link to="/"><Logo /></Link>
        <Link to="/"><Button variant="ghost"><ArrowLeft size={16} className="mr-1" /> Voltar</Button></Link>
      </header>
            ...
            <tbody>
              {rows.map((r) => (
                <tr key={r.student_id} className="border-t border-border">
                  <td className="p-3 font-bold">{r.rank <= 3 ? <Medal size={18} className="inline text-primary" /> : `#${r.rank}`}</td>
              ...
              {rows.length === 0 && (
                <tr><td colSpan={4} className="p-10 text-center text-muted-foreground"><Trophy className="mx-auto mb-2" /> Sem rankings ainda.</td></tr>
              )}
            </tbody>
          </table>
        </div>
